...
struct UsbVideoStreamerStats {
  ...
  int frameCount = 0; // total frames since last stats reset
...
class UsbVideoStreamer final {
 public:
  ...
  bool reconfigureStream(int32_t width, int32_t height, int32_t fps, uvc_frame_format format, ANativeWindow* previewWindow);
  bool start();
  bool stop();
