...
class UsbAudioStreamer final {
 public:
  ...
  UsbAudioStreamer(
      ...
      int desiredAltSetting = -1);      // USB alt setting number from Kotlin descriptor parsing
  ~UsbAudioStreamer();
  ...
  using AudioRecordCallback = std::function<void(const uint8_t*, size_t)>;
  void setAudioRecordCallback(AudioRecordCallback cb);
  ...
  // Audio levels for Java level meter (0.0 - 1.0), up to 8 channels
  float getAudioLevelLeft() const { return audioLevels_[0]; }
 ...
 private:
  ...
  steady_clock::time_point playbackStartTime_{};  // For initial mute period
  int initialMuteMs_{3000};  // 0 = no mute (reconnect), 3000 = mute (fresh plug)
  float audioLevels_[MAX_AUDIO_CHANNELS]{0.0f};
  std::string ctrlDiagLog_;   // Diagnostic log of control transfers for FileLogger
  int claimedControlInterface_{-1}; // Audio Control interface number
  std::string rawPktHexDump_; // First packet raw hex dump for debugging

  AudioRecordCallback audioRecordCallback_;
  std::mutex recordCallbackMutex_;

  static constexpr uint32_t kIsochronousTransferTimeoutMillis = 500;
  static constexpr uint8_t kInterfaceSubClassStreaming = 0x02;
  static constexpr uint8_t kInterfaceSubClassControl = 0x01;
};
