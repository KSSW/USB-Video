...
/**
 * Container view for video TextureView. Implemented as a FrameLayout which can
...
class VideoContainerView(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
  ...
  override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
    ...
    if (width > 0 && height > 0) {
      ...
      val containerW = abs(right - left).toFloat()
      val containerH = abs(bottom - top).toFloat()
      val scaleX = containerW / width
      val scaleY = containerH / height
