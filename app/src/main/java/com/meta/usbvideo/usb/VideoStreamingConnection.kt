...
private val SUPPORTED_VIDEO_FOURCC_FORMATS: Array<String> = arrayOf("YUY2", "NV12", "MJPG", "GBR24")
...
data class VideoFormat(
  ...
  fun toLibuvcFrameFormat(): LibuvcFrameFormat {
    return when (fourccFormat.uppercase().trim()) {
      "YUY2", "YUYV" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_YUYV
      "UYVY" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_UYVY
      "MJPG" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_MJPEG
      "NV12" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_NV12
      "RGB ", "RGB2", "RGB3", "RGB24", "RGBA", "XRGB" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_RGB
      "BGR ", "BGR2", "BGR3", "BGR24", "BGRA", "XBGR", "GBR24" -> LibuvcFrameFormat.UVC_FRAME_FORMAT_BGR
      else -> throw IllegalArgumentException("Unsupported fourcc format '$fourccFormat' (hex=${fourccFormat.toByteArray().joinToString(" ") { String.format("%02X", it.toInt() and 0xFF) }})")
    }
  }
}
...
class VSUncompressedFormatDescriptor(pack: ByteBuffer) {
  ...
  // the descriptor contains 16 bytes guidFormat; read all 16 bytes to detect non-ASCII
  // fourcc formats like GBR24 (GUID {7DEB36E4-4F52-CE11-9F53-0020AF0BA770})
  val fourccFormat: String = run {
    val fourccBytes = ByteArray(4) { pack.get() }
    val guidRemainder = ByteArray(12) { pack.get() }
    val fullGuid = fourccBytes + guidRemainder
    val gbr24Guid = byteArrayOf(
      0x7D.toByte(), 0xEB.toByte(), 0x36.toByte(), 0xE4.toByte(),
      0x4F.toByte(), 0x52.toByte(), 0xCE.toByte(), 0x11.toByte(),
      0x9F.toByte(), 0x53.toByte(), 0x00.toByte(), 0x20.toByte(),
      0xAF.toByte(), 0x0B.toByte(), 0xA7.toByte(), 0x70.toByte()
    )
    when {
      fullGuid.contentEquals(gbr24Guid) -> "GBR24"
      else -> CharArray(4) { (fourccBytes[it].toInt() and 0xFF).toChar() }.concatToString()
    }
  }
...
class VSFrameDescriptor(pack: ByteBuffer) {
  ...
  /** All supported frame intervals in 100ns units */
  private val frameIntervals: List<Int> = run {
    val intervals = mutableListOf<Int>()
    when {
      bFrameIntervalType == 0 -> {
        // Continuous frame intervals: min, max, step
        val dwMinFrameInterval = pack.getInt()
        val dwMaxFrameInterval = pack.getInt()
        val dwFrameIntervalStep = pack.getInt()
        if (dwFrameIntervalStep > 0) {
          var interval = dwMinFrameInterval
          while (interval <= dwMaxFrameInterval) {
            intervals.add(interval)
            interval += dwFrameIntervalStep
          }
        } else {
          intervals.add(dwDefaultFrameInterval)
        }
      }
      bFrameIntervalType > 0 -> {
        // Discrete frame intervals
        repeat(bFrameIntervalType) {
          intervals.add(pack.getInt())
        }
      }
      else -> intervals.add(dwDefaultFrameInterval)
    }
    intervals
  }
  ...
  /** All supported FPS values for this frame descriptor */
  fun fpsList(): List<Int> = frameIntervals.map { 10_000_000 / it }.distinct().sortedDescending()
